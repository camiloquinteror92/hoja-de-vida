...
window.SITE = {
  ui: {
    en: {
      ...
      fitLead: "Paste the job description and see, requirement by requirement, what I have done that matches.",
    ...
    es: {
      ...
      fitLead: "Pega la descripción de la vacante y mira, requisito por requisito, qué he hecho que responde a cada uno.",
  ...
  portfolio: {
    en: {
      ...
      closingLead: "Paste your job description and get an honest, requirement-by-requirement answer in two seconds.",
      cases: [
        {
          tag: "Performance",
    ...
    es: {
      ...
      closingLead: "Pega la descripción de la vacante y en dos segundos tienes una respuesta honesta, requisito por requisito.",
      cases: [
        {
          tag: "Rendimiento",
  ...
  matcher: {
    ui: {
      en: {
        ...
        lead: "Paste a job description. You'll see which requirements I cover, with evidence, and I'll be upfront about the gaps.",
        placeholder: "Paste the job description here",
        sample: "Try a sample job description",
        analyze: "Check the fit",
        privacy: "This runs entirely in your browser. Nothing you paste leaves this page.",
        empty: "Paste a job description first, or try the sample.",
        none: "I couldn't find technical requirements in that text. Try pasting the full job description.",
        fit: "fit",
        strong: "Strong match",
        partial: "Partial",
        gap: "Not yet",
      ...
      es: {
        ...
        lead: "Pega la descripción de una vacante. Verás qué requisitos cubro, con evidencia, y te diré con franqueza lo que no tengo.",
        placeholder: "Pega aquí la descripción de la vacante",
        sample: "Probar con una vacante de ejemplo",
        analyze: "Revisar el encaje",
        privacy: "Esto corre solo en tu navegador. Nada de lo que pegas sale de esta página.",
        empty: "Primero pega una descripción, o prueba con el ejemplo.",
        none: "No encontré requisitos técnicos en ese texto. Prueba pegando la descripción completa.",
        fit: "encaje",
    ...
    /* status: match | partial | gap. Cada regla se evalúa sobre la vacante pegada. */
    rules: [
      ...
      { id: "rest", status: "match", re: [/\brestful\b|\brest\s*apis?\b|\bapi\s*design\b|dise[ñn]o de apis/i, /\bREST\b/], en: ["REST API design", "Designed Owly's REST API, a public partner API and an inbound API that a bank calls."], es: ["Diseño de APIs REST", "Diseñé la API REST de Owly, una API pública para aliados y una API de entrada que llama un banco."] },
      ...
      { id: "postgres", status: "match", re: [/postgres|\bsql\b|relational databases?|bases? de datos relacional/i], en: ["PostgreSQL and SQL", "PostgreSQL 16 in production. Tuned the slowest views from about 17 s to under 1 s."], es: ["PostgreSQL y SQL", "PostgreSQL 16 en producción. Bajé las vistas más lentas de unos 17 s a menos de 1 s."] },
      ...
      { id: "redis", status: "match", re: [/\bredis\b/i], en: ["Redis", "Redis as Celery broker, cache and the WebSocket channel layer."], es: ["Redis", "Redis como broker de Celery, caché y capa de canales de WebSockets."] },
      { id: "queues", status: "match", re: [/\bcelery\b|task queues?|job queues?|background (jobs|tasks|workers)|colas? de tareas|asynchronous (jobs|tasks|processing)/i], en: ["Celery and background jobs", "Celery workers and scheduled jobs for integrations, notifications and syncs."], es: ["Celery y trabajos en segundo plano", "Workers de Celery y tareas programadas para integraciones, notificaciones y sincronizaciones."] },
      { id: "streaming", status: "partial", re: [/kafka|kinesis|rabbitmq|\bsqs\b|pub\s*\/?\s*sub|event[- ]driven|message (brokers?|queues?)/i], en: ["Messaging and event streams", "Queues and pub/sub on Redis (Celery, Channels). No Kafka or RabbitMQ in production yet."], es: ["Mensajería y eventos", "Colas y pub/sub sobre Redis (Celery, Channels). Todavía no uso Kafka ni RabbitMQ en producción."] },
      ...
      { id: "testing", status: "match", re: [/\btest(s|ing)?\b|pytest|\btdd\b|pruebas/i], en: ["Automated testing", "pytest suites and contract tests that fail the build in CI."], es: ["Pruebas automatizadas", "Suites de pytest y pruebas de contrato que rompen el build en CI."] },
      ...
      { id: "saas", status: "match", re: [/multi[- ]?tenan|\bsaas\b/i], en: ["Multi-tenant SaaS", "Designed Owly's tenant isolation, enforced by a CI check."], es: ["SaaS multi-tenant", "Diseñé el aislamiento por empresa de Owly, con un chequeo de CI que lo hace cumplir."] },
      ...
      { id: "fintech", status: "match", re: [/fintech|lending|\bloans?\b|credit|cr[eé]dito|payments?|\bpagos\b|banking|\bbanca\b|financial services|servicios financieros/i], en: ["Fintech and lending", "Two years on a lending platform at MO, financial engines at OWLY, and I'm a former investment analyst."], es: ["Fintech y crédito", "Dos años en una plataforma de crédito en MO, motores financieros en OWLY, y fui analista de inversiones."] },
      { id: "llm", status: "match", re: [/\bllms?\b|large language models?|generative ai|gen\s?ai|openai|anthropic|\bclaude\b|\bgpt|\bai\b|\bia\b|artificial intelligence|inteligencia artificial|\brag\b|embeddings?|vector (db|database|search)|agentic|ai agents?|agentes de ia/i], en: ["AI and LLMs", "LLM and voice agents in production (Claude, OpenAI, ElevenLabs) exposed over MCP, plus pgvector embeddings."], es: ["IA y LLMs", "Agentes de IA y de voz en producción (Claude, OpenAI, ElevenLabs) expuestos por MCP, y embeddings con pgvector."] },
      ...
      { id: "mobile", status: "gap", re: [/\bios\b|android|react native|flutter|\bswift\b/i], en: ["Mobile apps", "No native mobile work. My work is backend and web."], es: ["Apps móviles", "No he hecho apps nativas. Mi trabajo es backend y web."] },
      { id: "otherlang", status: "gap", re: [/\bgolang\b|\bgo\s*(developer|engineer|services?)\b|\bjava\b|node\.?js|\bruby\b|\bphp\b|laravel|\bc#|asp\.net|\bdotnet\b|\brust\b|\bscala\b|\belixir\b|\bkotlin\b/i], en: ["Other backend languages", "Not professionally. Python is my backend language."], es: ["Otros lenguajes backend", "No de forma profesional. Mi lenguaje backend es Python."] },
      { id: "cloud", status: "partial", re: [/\bgcp\b|google cloud|azure/i], en: ["GCP or Azure", "My cloud is AWS. The architecture patterns carry over."], es: ["GCP o Azure", "Mi nube es AWS. Los patrones de arquitectura se trasladan."] },
      ...
      { id: "agile", status: "match", re: [/\bagile\b|scrum|kanban|metodolog[ií]as? [aá]gil/i], en: ["Agile", "Agile sprints with Jira at MO. As CTO I run planning and delivery."], es: ["Metodologías ágiles", "Sprints ágiles con Jira en MO. Como CTO manejo la planeación y la entrega."] },
      { id: "leadership", status: "match", re: [/tech(nical)? lead|team lead|leadership|liderazgo|mentor|\bcto\b|head of engineering|engineering manager|ownership|l[ií]der t[eé]cnico|end to end|punta a punta/i], en: ["Ownership and leadership", "As CTO I own architecture, delivery and technical direction."], es: ["Liderazgo y ownership", "Como CTO soy dueño de la arquitectura, la entrega y la dirección técnica."] },
      { id: "startup", status: "match", re: [/startup|start-up|early[- ]stage|founding engineer|zero to one|0\s*(to|->|→)\s*1|greenfield|fast[- ]paced|emprendimiento/i], en: ["Startup experience", "Built Owly from the first commit, after 7 years founding and running startups."], es: ["Experiencia en startups", "Construí Owly desde el primer commit, después de 7 años fundando y dirigiendo startups."] },
      { id: "product", status: "match", re: [/product (mindset|sense|thinking|minded)|business (impact|outcomes?|acumen)|visi[oó]n de negocio/i], en: ["Business sense", "Economist and former investment analyst. I build for the business outcome."], es: ["Visión de negocio", "Economista y ex analista de inversiones. Construyo pensando en el resultado de negocio."] },
